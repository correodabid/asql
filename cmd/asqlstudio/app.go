...
import (
	...
	api "asql/internal/server/httpapi"
)
...
func (a *App) RecoveryCreateBackup(dataDir, backupDir string) (map[string]interface{}, error) {
	...
	manifest, err := executor.CreateBaseBackup(resolvedDataDir, strings.TrimSpace(backupDir))
	if err != nil {
		return nil, err
	}
...
func (a *App) RecoveryBackupManifest(backupDir string) (map[string]interface{}, error) {
	...
	manifest, err := executor.LoadBaseBackupManifest(strings.TrimSpace(backupDir))
	if err != nil {
		return nil, err
	}
	return structToMap(manifest)
...
func (a *App) RecoveryVerifyBackup(backupDir string) (map[string]interface{}, error) {
	...
	manifest, err := executor.VerifyBaseBackup(strings.TrimSpace(backupDir))
	if err != nil {
		return nil, err
	}
...
func (a *App) RecoveryRestoreLSN(backupDir, targetDataDir string, lsn uint64) (map[string]interface{}, error) {
	...
	result, err := executor.RestoreBaseBackupToLSN(a.reqCtx0(), strings.TrimSpace(backupDir), strings.TrimSpace(targetDataDir), lsn)
	if err != nil {
		return nil, err
	}
...
func (a *App) RecoveryRestoreTimestamp(backupDir, targetDataDir string, logicalTimestamp uint64) (map[string]interface{}, error) {
	...
	result, err := executor.RestoreBaseBackupToTimestamp(a.reqCtx0(), strings.TrimSpace(backupDir), strings.TrimSpace(targetDataDir), logicalTimestamp)
	if err != nil {
		return nil, err
	}
	return structToMap(result)
...
func (a *App) RecoverySnapshotCatalog(dataDir string) (map[string]interface{}, error) {
	...
	entries, err := executor.InspectDataDirSnapshotCatalog(resolvedDataDir)
	if err != nil {
		return nil, err
	}
	return map[string]interface{}{"snapshots": entries}, nil
}
...
func (a *App) RecoveryWALRetention(dataDir string) (map[string]interface{}, error) {
	...
	state, err := executor.InspectDataDirWALRetention(resolvedDataDir)
	if err != nil {
		return nil, err
	}
	if state.DataDir == "" {
