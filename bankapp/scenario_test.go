...
func TestScenarioCoversDeepASQLFeatures(t *testing.T) {
	...
	for _, marker := range []string{"CREATE ENTITY", "VERSIONED FOREIGN KEY"} {
	...
	if len(domains) < 4 {
		t.Fatalf("expected at least four domains, got %d", len(domains))
	}
}

func TestFixtureParsesAndMatchesScenarioIntent(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("fixtures", "banking-core-demo-v1.json"))
	if err != nil {
	...
	if fixture.Name == "" || len(fixture.Steps) < 6 {
	...
	for _, fragment := range []string{
		"CREATE ENTITY transfer_entity",
		"VERSIONED FOREIGN KEY",
		"INSERT INTO payments.transfer_requests",
		"UPDATE identity.customers SET risk_tier = 'enhanced'",
	} {
