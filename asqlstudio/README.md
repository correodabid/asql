...
- Studio now includes a runtime connection manager in the title bar and status bar so operators can retarget pgwire/admin endpoints without relaunching the desktop app, keep a recent-connection list with one-click reconnect for quick reuse of previous targets, and save named connection profiles for stable environments like local dev, demo clusters, or shared staging nodes.
