...
Targeted repeats on 2026-03-16 for the key crossover points using `-benchtime=1x -count=3`:

- Replay-tail crossover validation:
	- `BenchmarkEngineRestartReplayTailSweep/replay_only_tail_5000-8`: `21,087,416–26,148,000 ns/op`, `26,674,736–26,773,152 B/op`, `178,506–178,513 allocs/op`
	- `BenchmarkEngineRestartReplayTailSweep/persisted_snapshot_tail_5000-8`: `35,924,583–37,296,750 ns/op`, `31,855,520–31,954,128 B/op`, `202,080–202,085 allocs/op`
	- `BenchmarkEngineRestartReplayTailSweep/replay_only_tail_10000-8`: `63,890,542–65,928,833 ns/op`, `49,112,968–49,213,136 B/op`, `308,760–308,771 allocs/op`
	- `BenchmarkEngineRestartReplayTailSweep/persisted_snapshot_tail_10000-8`: `65,166,500–91,017,874 ns/op`, `59,327,960–59,427,016 B/op`, `348,824–348,831 allocs/op`
- Cadence crossover validation:
	- `BenchmarkEngineRestartSnapshotCadenceSweep/replay_only_total_10500-8`: `68,446,917–68,836,667 ns/op`, `49,114,224–49,216,344 B/op`, `308,763–308,766 allocs/op`
	- `BenchmarkEngineRestartSnapshotCadenceSweep/persisted_snapshot_total_10500_tail_500-8`: `42,322,751–43,042,792 ns/op`, `24,531,944–24,535,248 B/op`, `143,578–143,585 allocs/op`
	- `BenchmarkEngineRestartSnapshotCadenceSweep/replay_only_total_50500-8`: `199,304,625–210,200,792 ns/op`, `221,613,944–221,711,872 B/op`, `1,259,860–1,259,865 allocs/op`
	- `BenchmarkEngineRestartSnapshotCadenceSweep/persisted_snapshot_total_50500_tail_500-8`: `190,608,166–195,092,458 ns/op`, `118,774,616–118,898,376 B/op`, `664,070–664,074 allocs/op`

Natural-policy insert-heavy validation on 2026-03-16 after the direct decoder (`BenchmarkEngineRestartNaturalWorkloadCadenceSweep`, `-benchmem -benchtime=1x -count=2`):
	- `insert_heavy/replay_only_total_1000`: `9,505,542–11,412,042 ns/op`, `3,413,456–3,413,776 B/op`, `24,908–24,910 allocs/op`
	- `insert_heavy/policy_persisted_total_1000_tail_500`: `4,000,000–7,541,000 ns/op`, `1,197,712–1,201,552 B/op`, `4,238–4,250 allocs/op`
...
- `and_hash_lookup`: `24,817 ns/op` vs `and_full_scan`: `3,280,450 ns/op` (~`132x` faster with single indexed conjunct + residual post-filter)
- `and_index_intersection`: `164,983 ns/op` vs `and_full_scan`: `3,280,450 ns/op` (~`19.9x` faster with explicit multi-index conjunct intersection)
- `or_index_union`: `20,925 ns/op` vs `or_full_scan`: `3,011,825 ns/op` (~`144x` faster with indexed union planning)
- `in_hash_lookup`: `14,958 ns/op` vs `in_full_scan`: `24,065,233 ns/op` (~`1609x` faster with indexed literal-list selection in this sample)
- `or_partial_index_union`: `1,278,350 ns/op` vs `or_partial_full_scan`: `3,468,383 ns/op` (~`2.7x` faster with selective hybrid OR planning)
- `or_partial_broad_indexed_falls_back`: `7,341,258 ns/op` (`full-scan` chosen intentionally for broad hybrid OR with an oversized indexed branch)
- `not_index_complement`: `574,383 ns/op` vs `not_full_scan`: `2,647,167 ns/op` (~`4.6x` faster with indexed complement planning)
- `not_in_index_complement`: `559,667 ns/op` vs `not_in_full_scan`: `2,812,733 ns/op` (~`5.0x` faster with indexed `NOT IN` complement planning)
...
- EXPLAIN observability note: access plans now include `indexed_predicates` and `residual_predicate` when relevant, candidate entries expose `detail` plus `rejected_reason`, and heuristic crossover fallbacks can now surface `pruned_candidates` for broad hybrid `OR`, `NOT`, and `NOT IN` paths too, so planner output shows both why evaluated strategies lost and which broad compound paths were intentionally discarded before the final cost comparison.
...
- Current restart-path interpretation:
	...
	- final closure interpretation on 2026-03-16: the evidence is now good enough to stop treating snapshot load as an active optimization slice. The hot path is materially improved (`BenchmarkEngineReadPersistedSnapshotsFromDir` ~`175.7–198.8 µs/op`; indexed ~`304.1 µs/op`), natural-policy restart wins are stable across insert/update/delete-heavy workloads, and the remaining losses are confined to intentionally adversarial forced-tail placements or the head-snapshot best-case replay fixture. Those are still useful diagnostics, but no longer strong enough to justify keeping snapshot load open as the next engineering priority.
