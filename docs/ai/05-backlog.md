...
- Snapshot-load benchmarking is now better grounded after fixing the restart benchmark harness to clone the WAL/snapshot fixture per iteration and removing one extra deep copy during snapshot materialization; with those corrections, persisted-snapshot restart is much closer to replay-only and allocates materially less, but it is still not yet faster on the present fixture, so this item remains open as an optimization target rather than a closed win.
- Focused snapshot-load benchmarks now show the main persisted-restart hotspot is snapshot-directory read/decompress/decode/materialization (~`429 µs/op` on the current fixture), while `replayFromSnapshots` state restore is comparatively small (~`60 µs/op`).
- Within that snapshot-directory load, binary decode is now the largest measured in-process component (~`192 µs/op`, ~`470 KB/op`, ~`2.8k allocs/op`), ahead of zstd decompression (~`74 µs/op`) and snapshot materialization (~`49 µs/op`).
