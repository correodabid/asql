...
- Snapshot-load benchmarking is now better grounded after fixing the restart benchmark harness to clone the WAL/snapshot fixture per iteration, removing one extra deep copy during snapshot materialization, and reducing dictionary-string allocation in the binary decoder; with those corrections, persisted-snapshot restart allocates materially less than replay-only, but it is still not yet a clear time win on the present fixture, so this item remains open as an optimization target rather than a closed win.
- Focused snapshot-load benchmarks now show the main persisted-restart hotspot is snapshot-directory read/decompress/decode/materialization (~`377 µs/op` on the current fixture), while `replayFromSnapshots` state restore is comparatively small (~`60 µs/op`).
- Within that snapshot-directory load, binary decode remains the largest measured in-process component (~`140–166 µs/op`, ~`458 KB/op`, ~`1.7k allocs/op`), ahead of zstd decompression (~`74 µs/op`) and snapshot materialization (~`49 µs/op`).
- A follow-up row-map sizing tweak improved the isolated decode benchmark, but it has not yet translated into a clear persisted-restart win on the current fixture.
