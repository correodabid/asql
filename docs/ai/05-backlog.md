...
- A workload-cadence sweep at the default and medium anchors now suggests the snapshot crossover depends strongly on mutation mix: `insert_heavy` still loses at ~`1k` total mutations but wins clearly by ~`10.5k`, whereas `update_heavy` and `delete_heavy` are already roughly break-even or modestly better at the small anchor and gain larger absolute wins by the medium anchor (~`5.18 s → 4.77 s` for `update_heavy`, ~`3.81 s → 3.33 s` for `delete_heavy` on the current M1 spot checks).
