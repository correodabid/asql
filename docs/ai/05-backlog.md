...
- Within that snapshot-directory load, binary decode remains the largest measured in-process component (~`166 µs/op`, ~`458 KB/op`, ~`1.7k allocs/op`), ahead of zstd decompression (~`74 µs/op`) and snapshot materialization (~`49 µs/op`).
