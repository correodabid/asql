...
- Initial `-benchtime=1x` workload spot checks on the current M1 put `insert_heavy` restart around ~`16.6–20.9 ms`, but `update_heavy` and `delete_heavy` around ~`270–278 ms`, which is early evidence that mutation mix can dominate restart cost independently of just total row count.
