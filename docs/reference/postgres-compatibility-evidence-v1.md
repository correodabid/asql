...
| Non-correlated derived tables in `FROM` | `internal/engine/parser/parser_test.go`: `TestParseDerivedTableInFrom`, `TestParseDerivedTableJoin`, `TestParseJoinWithDerivedRightTable`; `internal/engine/executor/engine_test.go`: `TestDerivedTableWithWindowJoin` | Direct | Covers current alias-required derived-table shapes plus a window-function + join workflow. |
