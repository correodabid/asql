...
| Current read vs mutation/schema authorization under durable principals | `internal/engine/executor/engine_security_test.go`: `TestAuthorizePlanUsesAuthenticatedReadAndAdminMutationModel`; `internal/server/pgwire/server_test.go`: `TestPGWireCurrentReadAllowsAuthenticatedUsersButMutationsRequireAdmin` | Direct | Covers the current MVP privilege surface: authenticated enabled principals can perform current-state `SELECT`, while `ADMIN` is required for current DML, schema changes, and `EXPLAIN` over mutating statements. |
...
| Late-created principals can gain current-state historical access without backdating existence | `internal/server/pgwire/server_test.go`: `TestPGWireLateCreatedPrincipalCanBeGrantedHistoricalReadWithoutBackdating` | Direct | Covers the current-state rule explicitly: a principal created after the target data commit is denied before grant, allowed after current `SELECT_HISTORY` grant, and audited without inventing historical existence. |
| Session/setup compatibility shim (`current_database()`, `current_user`, `session_user`, `SHOW`, `version()`, `SET`) | `internal/server/pgwire/server_test.go`: `TestCatalogStartupIntrospectionQueries`, `TestShowUnknownParamFallbackWorksOnExtendedProtocol`, `TestMainstreamToolStartupFlows/psql_startup`, `TestMainstreamToolStartupFlows/dbeaver_datagrip_startup`, `TestMainstreamToolStartupFlows/pgadmin_startup_schema_browse`, `TestPGWireSessionIdentityReflectsAuthenticatedPrincipal` | Direct | Includes explicit coverage for generic non-`asql_*` `SHOW <param>` fallback, session no-op probes (`RESET`, `RESET ALL`, `DEALLOCATE`, `DEALLOCATE ALL`), dynamic session identity surfaces for durable principals, and the current pgAdmin-style `current_database()` / `current_schema()` startup probes. |
| Catalog/introspection shim (`current_setting`, `pg_is_in_recovery`, `pg_backend_pid`, `pg_database`, `pg_type`, `pg_settings`, `pg_namespace`, `pg_class`, `information_schema.tables`, grant-aware privilege probes) | `internal/server/pgwire/server_test.go`: `TestCatalogStartupIntrospectionQueries`, `TestCatalogEmptyInterceptsExposeSchemaAcrossProtocols`, `TestMainstreamToolStartupFlows/psql_startup`, `TestMainstreamToolStartupFlows/dbeaver_datagrip_startup`, `TestMainstreamToolStartupFlows/pgx_go_driver_startup`, `TestMainstreamToolStartupFlows/pgadmin_startup_schema_browse`, `TestPGWirePrivilegeProbesReflectDurablePrincipalGrants` | Direct | Mainstream introspection paths are covered, including schema-stable empty-result intercepts for `pg_index`, `pg_constraint`, `pg_proc`, `pg_am`, `pg_extension`, `pg_roles`, `pg_authid`, and `pg_user`, principal-aware `has_*_privilege(...)` behavior for the currently documented durable-principal privilege surface, and the current pgAdmin-style schema-browse subset over `pg_namespace`, `pg_class`, and `information_schema.tables`. |
