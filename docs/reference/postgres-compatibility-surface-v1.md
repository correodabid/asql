...
- Extended-query behavior:
  ...
  - `Describe Statement` returns `ParameterDescription`; common schema-aligned `INSERT ... VALUES ($n, ...)`, `UPDATE ... SET col = $n`, and simple `WHERE column OP $n` predicate shapes can advertise concrete scalar OIDs instead of only unspecified (`0`) entries
